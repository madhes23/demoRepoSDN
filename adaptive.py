...
class ShareIt(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):
        	...
        	self.mac_to_port = {}
		self.servers = []
		self.servers.append({'ip':"10.0.0.1", 'mac':"00:00:00:00:00:01",'server_port':1})
		self.servers.append({'ip':"10.0.0.2", 'mac':"00:00:00:00:00:02",'server_port':2})
		self.servers.append({'ip':"10.0.0.3", 'mac':"00:00:00:00:00:03",'server_port':3})
		...
		self.test_thread=hub.spawn(self._continuous_packet_send())"""

	@set_ev_cls(ofp_event.EventOFPPacketIn,[MAIN_DISPATCHER, DEAD_DISPATCHER])
	def _state_change_handler(self, ev):
	    datapath = ev.msg.datapath
	    self.datapaths[datapath.id] = ev
	...
	def _monitor(self):
             while True:
                ...
                minimumrestime=min(self.elapsed_time)
                maximumrestime=max(self.elapsed_time)
                restime=abs( maximumrestime - minimumrestime)
		print(restime)
                if restime == 0 :
                   restime=0.1
                global_1= .25
                ...
                diff=0
                if restime < global_1 :
                    var= global_1 /restime
                    i=0
                    while i < var :
                        diff=diff+restime
                        i=i+1
                elif restime > global_1 :
                    var= restime/global_1
                    i=0
                    while i < var :
                        diff=diff-restime
                        i=i+1
                    ...
                    diff=restime
		print (diff,"________________________________________")
                hub.sleep(diff)

	def _request_stats(self, ev):
		msg = ev.msg
		...
		tha = ["00:00:00:00:00:01","00:00:00:00:00:02","00:00:00:00:00:03"]
		tpa = ["10.0.0.1","10.0.0.2","10.0.0.3"]
		ether_type = 2054 #ethertype ARP
		pkt = [ "" for i in range(0,3)]
		ether_frame = [ "" for i in range(0,3)]
		arp_req_frame = [ "" for i in range(0,3)]
		pout = [ "" for i in range(0,3)]
		for i in range(0,3):
			pkt[i] = packet.Packet()
			ether_frame[i] = ethernet.ethernet(tha[i], sha, ether_type)
			pkt[i].add_protocol(ether_frame[i])
			arp_req_frame[i] = arp.arp(hrdw_type, protocol, hrdw_add_len, prot_add_len, opcode, sha, spa, tha[i], tpa[i])
			pkt[i].add_protocol(arp_req_frame[i])
			pkt[i].serialize()
			actions = [parser.OFPActionOutput(i+1)]
			pout[i] = parser.OFPPacketOut(datapath=datapath, in_port=ofproto.OFPP_ANY, data=pkt[i].data, actions=actions, 	buffer_id=0xffffffff)   
			self.start_time[i] = time.time()
			self.logger.info("packet-out %s" % (pout[i]))
			datapath.send_msg(pout[i])
	...
	def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        """def _continuous_packet_send(self):
		        try:
			    self.ingress_event=hub.Event()
			    tid=hub.spawn(self._send_packet_thread)
			    self.ingress_threads.append(tid)
			    print(self.ingress_threads)
			    self.ingress_event.wait(10)
	    		finally:
			    print("_______")	
	def _send_packet_thread(self):
	   print("hello")"""
	...
	def _packet_in_handler(self, ev):
		...
		if eth.ethertype == 2054:
			...
			if ((arp_head.src_ip == "10.0.0.1") and (arp_head.dst_ip == "10.0.0.100")):
			    self.elapsed_time[0] = time.time() - self.start_time[0]
			    print "ELAPSED Server 1 ::: ", self.elapsed_time[0]
			elif ((arp_head.src_ip == "10.0.0.2") and (arp_head.dst_ip == "10.0.0.100")):
			    self.elapsed_time[1] = time.time() - self.start_time[1]
			    print "ELAPSED Server  2 ::: ", self.elapsed_time[1]
			elif ((arp_head.src_ip == "10.0.0.3") and (arp_head.dst_ip == "10.0.0.100")):
			    self.elapsed_time[2] = time.time() - self.start_time[2]
			    print "ELAPSED Server  3 ::: ", self.elapsed_time[2]		    
			if arp_head.dst_ip == self.dummyIP:
				...
				buffer_id = ofproto.OFP_NO_BUFFER #id assigned by datapath - keep track of buffered packet
		...
		minimumServer = self.elapsed_time.index(min(self.elapsed_time))
	
			


		choice_ip = self.servers[minimumServer]['ip']
		choice_mac = self.servers[minimumServer]['mac']
		choice_server_port = self.servers[minimumServer]['server_port']
